...
- **内置阅读器**：原文/译文/双语对照随时切换，**连续滚动阅读**（懒加载）；目录导航；选中文字即可**添加批注**（持久保存、点击跳转、**一键导出 Markdown 笔记**）或 **AI 解释**（结合论文上下文）
- **术语表**：界面维护术语译法（如 Transformer 不译），翻译全文一致
- **AI 助手「小鱼」**：右下角 DeepSeek 鲸鱼女仆桌宠，随时提问论文/工具使用问题
- **深色模式 / 访问密码**：夜间阅读护眼；局域网访问可开启密码保护
...
**桌面应用**（推荐）：双击 **`论文助手.bat`**（开发态）或 **`dist/论文助手/论文助手.exe`**（打包版）——弹出独立桌面窗口，无需浏览器。窗口关闭即退出。

**浏览器模式**：双击 **`start.bat`** —— 首次运行会自动创建虚拟环境、安装依赖并生成 `.env` 配置模板（用记事本填入 API Key 后再双击一次即可），浏览器自动打开。
